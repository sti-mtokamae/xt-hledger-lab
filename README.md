...
| **src/xt_hledger/core.clj** | XTDB + hledger 統合ロジック（WIP） |
...
### 0. PostgreSQL 準備（XTDB v2 は PostgreSQL をネイティブデータベースとして使用）
...
### Phase 1: XTDB v2 の基本（Week 1-2）
- [ ] PostgreSQL 接続セットアップ
- [ ] ドキュメント投入（put/update/delete）
- [ ] 基本的なDatalogクエリ
- [ ] Bitemporal な遡及修正
- **Examples**: `examples/02_xtdb_v2_basics.clj`, `examples/03_xtdb_v2_postgresql.clj`

### Phase 2: XTDB の高度な機能（Week 3-4）
- [ ] グラフ構造（再帰クエリ）
- [ ] XTQL（v2 SQL風クエリ）
- [ ] トランザクション関数による制約

### Phase 3: hledger との連携（Week 5-6）
- [ ] XTDB → hledger 形式の変換ロジック
- [ ] 複式簿記データの整合性チェック
- [ ] マルチカレンシー対応
...
### 2. PostgreSQL を起動（ターミナル別窓）

```bash
# Docker を使用する場合
docker run --rm -e POSTGRES_PASSWORD=password \
  -p 5432:5432 -e POSTGRES_DB=xtdb_dev \
  postgres:16
```

### 3. hledger でテスト（即座に動作）
...
### 4. XTDB v2 + PostgreSQL チュートリアル

```bash
# PostgreSQL 対応版（推奨）
# ※ PostgreSQL が localhost:5432 で起動していることを確認
rlwrap clj -i examples/03_xtdb_v2_postgresql.clj

# または インメモリ版（PostgreSQL 不要）
rlwrap clj -i examples/02_xtdb_v2_basics.clj
```

### XTDB v2 + PostgreSQL コード例
