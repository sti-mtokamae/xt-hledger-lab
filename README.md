...
| **Phase 3** | REST API 層（Clojure + Ring） | ⏳ 計画中 | - |
| **Phase 4** | postgres-source 統合（オプション） | ⏳ 様子見 | - |
...
**Phase 3（計画中）: REST API 層（Clojure + Ring）**
- Clojure （Ring） で HTTP サーバー構築（ビジネスロジック層）
- **認証・認可**は API Gateway（API マネージャー）が担当（別層）
- **読取**：REST API → SQL → XTDB（参照のみ）
- **更新**：REST API → Clojure API → put-documents + sync（制御された更新）
- 単方向同期で十分：XTDB → PostgreSQL（Phase 2.5 と同じパターン）
- **アーキテクチャ**：
  ```
  [クライアント] → [API Gateway (認証・認可)] → [Clojure/Ring (ビジネスロジック)]
  ```
...
※ 将来的な拡張オプション。複数外部アプリが PostgreSQL を更新する場合に検討。
XTDB の postgres-source モジュール（CDC 機能）を活用した自動同期。
現状は Phase 3 アーキテクチャで単一 Clojure 主導なら十分。
