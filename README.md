...
| **Phase 2.5** | PostgreSQL バックアップ同期（ハイブリッド） | ✅ 完了 | `d48f5d0` |
| **Phase 3** | Spring Boot 統合（単方向） | ⏳ 計画中 | - |
| **Phase 4** | postgres-source 統合（双方向）※ | ⏳ 様子見 | - |
...
; Phase 2.5: ファイル永続化 + PostgreSQL バックアップ（ハイブリッド）
(with-open [node (core/create-node :db-path "./my-db")]
  (core/demo-phase2-persistent node)
  (core/sync-documents-to-postgres node :password "password"))
```

### 拡張性：Phase 3 ～ Phase 4 の方向性

**Phase 3（計画中）: Spring Boot 統合 + プログラム的中間層**
- Clojure アプリが引き続き主導
- **読取**：REST API → SQL → XTDB（参照のみ）
- **更新**：REST API → Clojure API → put-documents + sync（制御された更新）
- 単方向同期で十分：XTDB → PostgreSQL（Phase 2.5 と同じパターン）
- **メリット**：双方向同期不要、プログラム層で整合性管理

```
┌─ 読取フロー ──────────────────┐
│  REST API (Spring Boot)       │
│    ↓ SQL query                │
│  XTDB ← PostgreSQL バックアップ │
│    ↓ JSON                     │
│  クライアント                  │
└───────────────────────────────┘
...
**Phase 4（様子見中）: postgres-source 統合**

XTDB が **postgres-source** をリリースしたら、複数外部アプリ対応として有効化予定。
- 現在は「Phase 3 のプログラム的管理で十分」判断
- 将来、外部 ERP など複数アプリ連携が必要になったら検討

```
複数のアプリ（ERP等）
  ↓ 更新
PostgreSQL 共通テーブル
  ↓ CDC (postgres-source 監視)
XTDB (リアルタイム同期)
  ↓ クエリ
hledger + 監査ログ
```

**postgres-source とは何か**
- XTDB 公式モジュール（現在 GitHub に存在、Maven Central 未公開）
- 機能：PostgreSQL の Logical Replication（変更捕捉）を監視
- 用途：外部アプリが PostgreSQL を更新 → XTDB に自動同期

**いつ必要になるか**
- ✅ 単一 Clojure アプリが主導：Phase 2.5 で十分
- ❌ 複数アプリ + 共通 PostgreSQL：postgres-source 必須
- ❌ マイクロサービス化：postgres-source 推奨

**現状と戦略**
- **Phase 2.5**：ファイル永続化 + 単方向同期（現在）✅ 動作確認済
- **Phase 3**：REST API 層で読取と更新を分離（計画中）
  - 読取：SQL クエリ（XTDB から直接）
  - 更新：Clojure API（制御フロー明確）
  - 効果：複雑な CDC 不要、プログラム層で整合性管理
- **Phase 4**：postgres-source 統合（様子見）
  - 用途：複数外部アプリが PostgreSQL 更新する場合
  - 現判断：Phase 3 アーキテクチャで単一主導アプリ対応は可能
  - 予定：Maven Central リリース + 必要性出現時に有効化

**なぜ Phase 3 で十分か**
```
postgres-source の利点：複数アプリ → PostgreSQL → XTDB（自動同期）
Phase 3 アーキテクチャ：複数アプリ → REST API → Clojure → XTDB + PG（制御同期）

→ 後者は「プログラム的中間層で整合性管理」
→ 単一 Clojure アプリが主導なら、複雑な CDC は不要
```
